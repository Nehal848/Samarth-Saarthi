...
const statusColors = { open: "bg-success/10 text-success border-success/20", in_progress: "bg-warning/10 text-warning border-warning/20", completed: "bg-muted text-muted-foreground border-border" };
...
const submissionStatusColors = {
  ...
  unselected: "bg-muted text-muted-foreground border-border"
};
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground">Complete tasks, earn credits. Every submission AI-verified.</p>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" size="sm"><Filter className="w-4 h-4 mr-2" />Filter</Button>
            <Dialog>
              <DialogTrigger asChild><Button className="gradient-bg text-white"><Plus className="w-4 h-4 mr-2" />Post Bounty</Button></DialogTrigger>
              <DialogContent>
                ...
                <div className="space-y-4 pt-4">
                  ...
                  <Button className="w-full gradient-bg text-white" disabled={!title || !description || !reward}>Post Bounty</Button>
                </div>
              </DialogContent>
            </Dialog>
          </div>
              ...
              <motion.div key={bounty.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
                <Card className="glass-card hover:shadow-card-hover transition-all">
                  <CardContent className="pt-6">
                    ...
                    {bounty.status === "open" && (
                      <Dialog>
                        <DialogTrigger asChild><Button size="sm" className="gradient-bg text-white w-full">Submit Proof</Button></DialogTrigger>
                        <DialogContent>
                      ...
                      <div key={sub.id} className="p-4 border rounded-lg">
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">Submission by {sub.userName}</p>
                          </div>
        ...
        <Dialog open={showTipDialog} onOpenChange={setShowTipDialog}>
          <DialogContent>
            ...
            <div className="space-y-4 pt-4">
              <div>
                <p className="text-sm mb-2">Reward exceptional work with a tip</p>
                <div className="flex items-center gap-2">
                  <Input 
                    ...
                    min="0"
                  />
                  <span className="text-sm">max {Math.floor((selectedBounty?.reward || 0) * 0.5)} credits</span>
                </div>
