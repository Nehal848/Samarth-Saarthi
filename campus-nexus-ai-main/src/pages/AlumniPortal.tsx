    ...
    <PageWrapper className="bg-background">
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2"><Users className="w-6 h-6 text-primary" />Alumni Portal</h1>
            <p className="text-muted-foreground">Discover verified talent. High signal, zero noise.</p>
          </div>
          <Badge variant="outline" className="text-muted-foreground">Read-only access</Badge>
        </div>

        <div className="grid lg:grid-cols-4 gap-6">
          {/* Filters */}
          <Card className="glass-card h-fit">
            <CardHeader><CardTitle className="text-base flex items-center gap-2"><Filter className="w-4 h-4" />Filters</CardTitle></CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="space-y-3">
                <label className="text-sm font-medium">Min AI Score: {minScore[0]}%</label>
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium">Communication Level</label>
                <div className="flex flex-col gap-2">
                  <Button 
            ...
            {mockStudents
              ...
              .map((student, i) => (
                <motion.div key={student.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
                  <Card 
                    className="glass-card hover:shadow-card-hover transition-all cursor-pointer"
                    onClick={() => {
                    ...
                    <CardContent className="pt-6">
                      <div className="flex flex-col md:flex-row md:items-center gap-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1"><h3 className="font-semibold text-lg">{student.name}</h3><Badge variant="secondary">{student.skillDNA}</Badge></div>
                          <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                            <span className="flex items-center gap-1"><Star className="w-4 h-4 text-karma" />{student.trustScore} Trust</span>
                            <span className="flex items-center gap-1"><BadgeCheck className="w-4 h-4 text-primary" />{student.badges} badges</span>
                            <span className="flex items-center gap-1"><Target className="w-4 h-4 text-success" />{student.bounties} bounties</span>
                            <span className="flex items-center gap-1"><ArrowLeftRight className="w-4 h-4 text-accent" />{student.swaps} swaps</span>
                            <span className="flex items-center gap-1"><BadgeCheck className="w-4 h-4 text-success" />{student.communicationLevel}</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-4">
                          <ScoreRing score={student.avgScore} size="sm" />
                          <Dialog>
                            <DialogTrigger asChild><Button className="gradient-bg text-white" onClick={(e) => {
                              e.stopPropagation();
                              setSelectedStudent(student);
                            }}><Calendar className="w-4 h-4 mr-2" />Schedule Meet</Button></DialogTrigger>
                            <DialogContent>
                              <DialogHeader><DialogTitle>Schedule Meeting with {selectedStudent?.name}</DialogTitle></DialogHeader>
                              <div className="space-y-4 pt-4">
                                ...
                                <div className="space-y-3">
                                  ...
                                  <div className="p-3 rounded-lg border bg-muted/30">
                                    ...
                                    <p className="text-xs text-warning mt-1">(Hidden unless approved)</p>
                                  </div>
                                </div>
                                <div className="p-4 rounded-xl border border-dashed border-border text-center">
                                  <Calendar className="w-8 h-8 mx-auto mb-2 text-muted-foreground" />
                                  <p className="text-sm text-muted-foreground mb-3">Calendar integration coming soon</p>
                                  <Button variant="outline" className="gap-2"><ExternalLink className="w-4 h-4" />Open Google Meet</Button>
                                </div>
                              </div>
                            </DialogContent>
                          </Dialog>
