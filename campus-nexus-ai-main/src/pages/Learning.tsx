    ...
    <PageWrapper className="bg-background">
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2"><GraduationCap className="w-6 h-6 text-primary" />Learning Marketplace</h1>
            ...
            <Dialog>
              ...
              <DialogContent>
                ...
                <div className="space-y-4 pt-4">
                  ...
                  <Button className="w-full gradient-bg text-white" disabled={!learnSkill || !duration} onClick={handlePostLearning}>Post Learning Session</Button>
                </div>
              </DialogContent>
            </Dialog>
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {mockLearningSessions.map((session, i) => (
            <motion.div key={session.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
              <Card className="glass-card hover:shadow-card-hover transition-all">
                <CardContent className="pt-6">
                  <div className="flex items-center gap-3 mb-4">
                    <Avatar><AvatarImage src={session.user.avatar} /><AvatarFallback>{session.user.name.charAt(0)}</AvatarFallback></Avatar>
                    <div><p className="font-medium">{session.user.name}</p><div className="flex items-center gap-1 text-sm text-muted-foreground"><Star className="w-3 h-3 text-karma fill-karma" />{session.trustScore}</div></div>
                  </div>
                  <div className="flex items-center gap-2 mb-4">
                    <Badge variant="secondary" className="bg-success/10 text-success border-success/20">Teaching: {session.skill}</Badge>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="flex items-center gap-1 text-sm font-semibold text-[hsl(var(--alpha-credits))]">
                        <Coins className="w-4 h-4" />{session.alphaCredits} α
                      </div>
                    </div>
                    <Button size="sm" className="gradient-bg text-white">Join Session</Button>
                  </div>
                </CardContent>
              </Card>
            </motion.div>
        ...
        <Card className="glass-card mt-8">
          <CardHeader><CardTitle className="flex items-center gap-2"><Video className="w-5 h-5 text-primary" />Active Session</CardTitle></CardHeader>
          <CardContent className="text-center py-8">
            <p className="text-muted-foreground mb-4">No active sessions. Accept a swap to get started!</p>
            <Badge variant="outline">Session will appear here with Google Meet link</Badge>
          </CardContent>
        </Card>
        
        {/* Completed Sessions with Feedback */}
        <Card className="glass-card mt-8">
          <CardHeader><CardTitle className="flex items-center gap-2">Completed Sessions</CardTitle></CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="p-4 border rounded-lg">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <h4 className="font-medium">Python to React.js Swap</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
                  <div className="space-y-2">
                    <h5 className="text-sm font-medium text-muted-foreground">Teacher Quality</h5>
                  ...
                  <div className="space-y-2">
                    <h5 className="text-sm font-medium text-muted-foreground">Match Quality</h5>
                    <div className="flex gap-1">
                      {[...Array(5)].map((_, i) => (
                  ...
                  <div className="space-y-2">
                    <h5 className="text-sm font-medium text-muted-foreground">AI Content Rating</h5>
                    <div className="flex gap-1">
                      {[...Array(5)].map((_, i) => (
                        <Star key={i} className={`w-4 h-4 ${i < 4 ? 'text-yellow-500 fill-yellow-500' : 'text-muted-foreground'}`} />
                      ))}
